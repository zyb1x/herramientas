                ...
                <div class="mt-6 flow-root sm:mt-8">
                    @if (empty($prestamos))
                        <div class="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p class="text-base font-medium text-orange-500">No tienes préstamos registrados.</p>
                        </div>
                    @else
                            ...
                            @foreach ($prestamos as $prestamo)
                                <div class="prestamo-item grid md:grid-cols-12 gap-4 md:gap-6 py-4 md:py-6"
                                    ...
                                    <dl class="md:col-span-3 order-2 md:order-1">
                                        <dt class="text-medium font-medium uppercase text-orange-400">#Id del pedido:</dt>
                                        <dd class="mt-1 text-base font-semibold text-[#023047]">
                                            #{{ str_pad($prestamo['id_prestamo'], 5, '0', STR_PAD_LEFT) }}
                                        </dd>
                                        <dt class="mt-2 text-medium font-medium uppercase text-orange-400">Fecha:</dt>
                                    ...
                                    <dl class="md:col-span-6 order-3 md:order-2 content-center">
                                        <dt class="text-mendium font-medium uppercase text-orange-400">Empleado:</dt>
                                        <dd class="mt-1 text-base font-semibold text-[#023047]">
                                            {{ $prestamo['empleado']['nombre'] ?? 'Sin nombre' }}
                                            {{ $prestamo['empleado']['apellido_p'] ?? '' }}
                                            {{ $prestamo['empleado']['apellido_m'] ?? '' }}
                                        </dd>
                                        <dt class="mt-2 text-medium font-medium uppercase text-orange-400">Registrado por:
                                        </dt>
                                        <dd class="mt-1 text-sm text-[#023047] font-bold">
                                    ...
                                    <div
                                        ...
                                        @php
                                            ...
                                            $hasHerramientas = !empty(
                                                array_filter(
                                                    $prestamo['detalles'] ?? [],
                                                    fn($d) => !empty($d['id_herramienta']),
                                                )
                                            );
                                            $hasMateriales = !empty(
                                                array_filter(
                                                    $prestamo['detalles'] ?? [],
                                                    fn($d) => !empty($d['id_material']),
                                                )
                                            );
                                        @endphp
                                        ...
                                        <div class="flex gap-2 mb-2">
                                            @if ($hasHerramientas)
                                                <span
                                                    class="inline-flex items-center rounded-lg bg-gray-200 px-2.5 py-1 text-xs font-semibold text-gray-800">
                                                    🛠 Herramientas
                                                </span>
                                            @endif
                                            @if ($hasMateriales)
                                                <span
                                                    class="inline-flex items-center rounded-lg bg-orange-100 px-2.5 py-1 text-xs font-semibold text-orange-800">
                                                    📦 Material
                                                </span>
                                            @endif
                                        </div>
                                        ...
                                        <div class="flex gap-2 mt-2">
                                            @if ($prestamo['estatus_general'] !== 'Cerrado' && $hasHerramientas)
                                                <form action="{{ route('devoluciones.index') }}" method="GET">
                                                    <input type="hidden" name="id_prestamo"
                                                        value="{{ $prestamo['id_prestamo'] }}">
                                                    <button type="submit"
                                                        class="inline-flex items-center gap-1.5 rounded-lg border border-transparent px-3 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 duration-150">
                                            ...
                                            <a href="{{ route('prestamos.show', $prestamo['id_prestamo']) }}"
                                                class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium bg-orange-500 hover:bg-[#023047] text-white hover:text-orange-500 duration-150">
